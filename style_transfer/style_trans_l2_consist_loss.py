...
parser.add_argument('--trainpics', default='output_style_l2_consist/',
                    help='folder to output training images')
parser.add_argument('--validationpics', default='output_style_l2_consist/',
                    help='folder to output validation images')
parser.add_argument('--testPics', default='output_style_l2_consist/',
                    help='folder to output test images')
parser.add_argument('--runfolder', default='output_style_l2_consist/',
                    help='folder to output test images')
parser.add_argument('--outckpts', default='output_style_l2_consist/',
                    help='folder to output checkpoints')
parser.add_argument('--outlogs', default='output_style_l2_consist/',
                    help='folder to output images')
parser.add_argument('--outcodes', default='output_style_l2_consist/',
...
def train(train_loader, epoch, Remover, Hnet, Enet):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        if opt.cuda:
            ...
            cover_img_B = cover_img_B.cuda() # b
        ...
        # diag_elements = 0.01 + (torch.rand(H) * 0.09)
        ...
        err_mse = mse_loss(trans_rev_secret_img, clean_imgv)
        ...
        err_remover = opt.betamse * err_mse + opt.betaconsist* err_consist + opt.betamse * pixel_loss
        ...
        start_time = time.time()
        # log writing
        log = '[%d/%d][%d/%d]\tLoss_mse: %.4f  Loss_l1: %.4f Loss_consist: %.4f Loss_sum: %.4f \tdatatime: %.4f \tbatchtime: %.4f' % (
            epoch, opt.niter, i, len(train_loader),
              RemoverMselosses.val, RemoverPixellosses.val, RemoverConsistlosses.val, Removerlosses.val, data_time.val,
            batch_time.val)
        ...
        if epoch % 1 == 0 and i % opt.resultPicFrequency == 0:
            save_result_pic(this_batch_size, cover_img_A, cover_imgv.data, container_img.data, uncover_img.data,
                                            secret_img, origin_rev_secret_img.data, rev_secret_img.data, trans_rev_secret_img.data,  
...
def validation(val_loader, epoch, Remover, Hnet, Enet):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(val_loader, 0):
            ...
            copyright_imgv = Variable(copyright_img)
            A_imgv = Variable(cover_img_A)
            container_img = Hnet(concat_imgv)  # B'
            remover_concate_img = torch.cat([container_img, clean_imgv], dim=1)
            remover_concate_imgv = Variable(remover_concate_img)
            origin_rev_secret_img = Enet(container_img)


            uncover_img = Remover(remover_concate_imgv) # B''
            rev_secret_img = Enet(uncover_img) 
    ...
    val_log = "validation[%d] val_mseloss = %.6f\t val_l1loss = %.6f\t val_consistloss = %.6f\t val_sumloss = %.6f\t validation time=%.2f" % (
        epoch,  RemoverMselosses.avg, RemoverPixellosses.avg, RemoverConsistlosses.avg, Removerlosses.avg, val_time)
